...
for sample in samples:
    ...
    if isMC:
        ...
        if sample.year==2018:
            modules = "lumiMask(year = "+str(sample.year)+"), MET_Filter(year = "+str(sample.year)+"), preselection(), SampleIdx("+ str(dictSample[sample.label]) +"), nanoTopcand(isMC=0), globalvar(), nanoTopevaluate_MultiScore(year = "+str(sample.year)+")"
        if sample.year==2022:
