...
vars.append(variable(name = "PuppiMET_T1_pt_nominal", title= "p_{T}^{miss}(Puppi) nominal [GeV]", nbins = 13, xarray = np.array([100, 125, 150, 175, 200, 225, 250, 275, 300, 350, 400, 500, 600, 1000], dtype = 'd')))#,xmin = 100, xmax=800))
vars.append(variable(name = "MHT", title= "MHT [GeV]", nbins = 9, xarray = np.array([0, 50, 100, 150, 200, 250, 300, 400, 500, 600], dtype = 'd')))#,xmin = 100, xmax=800))
...
var2d = [] 
var2d.append(variable2D(name = "PuppiMET_T1_pt_nominalVsMHT", xname = "PuppiMET_T1_pt_nominal", yname = "MHT", xtitle = " Puppi MET p_{T}", ytitle = "MHT", nxbins = 13, xarray = np.array([100, 125, 150, 175, 200, 225, 250, 275, 300, 350, 400, 500, 600, 1000], dtype = 'd'),
...
for v in [var[2]]:
    ...
    legend.AddEntry(h_eff_bkg, "Simulation", "Pl")
...
for v in var2d:
    ...
    h_eff_bkg.SetMarkerStyle(20)
    h_eff_bkg.SetMarkerSize(0.8)
    h_eff_bkg.GetXaxis().SetTitle(v._xtitle)
    h_eff_bkg.GetYaxis().SetTitle(v._ytitle)
    h_eff_bkg.GetZaxis().SetTitle("Efficiency")
    # h_eff_bkg.GetYaxis().SetRangeUser(0, 1.4)
    h_eff_bkg.GetYaxis().SetLabelFont(42)
    h_eff_bkg.GetYaxis().SetTitleFont(42)
    h_eff_bkg.GetXaxis().SetTitleSize(0.05)
    ...
    h_eff_data.SetMarkerStyle(20)
    h_eff_data.SetMarkerSize(0.8)
    h_eff_data.GetXaxis().SetTitle(v._xtitle)
    h_eff_data.GetYaxis().SetTitle(v._ytitle)
    h_eff_data.GetZaxis().SetTitle("Efficiency")
    h_eff_data.Draw("PE, same")
    CMS_lumi(pad1, lumi_sqrtS, iPos, "")
    legend = ROOT.TLegend(0.7, 0.7, 0.9, 0.9)
    legend.AddEntry(h_eff_bkg, "Simulation", "Pl")
    ...
    h_ratio.SetMarkerStyle(20)
    h_ratio.SetMarkerSize(0.8)
    h_ratio.GetXaxis().SetTitle(v._xtitle)
    h_ratio.GetYaxis().SetTitle(v._ytitle)
    h_ratio.GetZaxis().SetTitle("SF")
    # h_ratio.GetYaxis().SetRangeUser(0.6, 2.0)
    h_ratio.GetYaxis().SetLabelFont(42)
    h_ratio.GetYaxis().SetTitleFont(42)
    h_ratio.GetXaxis().SetTitleSize(0.05)
