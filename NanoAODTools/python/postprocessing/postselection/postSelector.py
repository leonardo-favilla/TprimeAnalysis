...
def preselection(df, btagAlg, year, EE):
    ...
    df = df.Define("GoodJet_idx", "GetGoodJet(Jet_pt_nominal, Jet_eta, Jet_passJetIdTightLepVeto)")
    df = df.Define("nGoodJet", "nGoodJet(GoodJet_idx)") 
    df = df.Define("GoodFatJet_idx", "GetGoodFatJet(FatJet_pt_nominal, FatJet_eta, FatJet_jetId)")
    ...
    df = df.Define("nForwardJet", "nForwardJet(Jet_pt_nominal, Jet_passJetIdTight, Jet_eta)")
